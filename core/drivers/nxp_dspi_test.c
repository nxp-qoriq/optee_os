...
#define DSPI_CTAR_DEFAULT_VALUE \
	(DSPI_CTAR_FMSZ(7) | DSPI_CTAR_PA_SCK(0) | DSPI_CTAR_P_DT(0) | \
	 DSPI_CTAR_CS_SCK(0) | DSPI_CTAR_A_SCK(0) | DSPI_CTAR_A_DT(0))
...
static TEE_Result dspi_test_suite(void)
{
	...
	if (status == TEE_SUCCESS) {
		EMSG("DSPI Base: 0x%lx", dspi_data.base);
		/* stop DSPI controller */
		dspi_data.chip.ops->end(&dspi_data.chip);
		/* configure DSPI chip instance */
		dspi_data.chip.ops->configure(&dspi_data.chip);
		/* start DSPI controller */
		dspi_data.chip.ops->start(&dspi_data.chip);

		for (j = 0; j < 1; j++) {
			EMSG("DSPI test loop: %zu", j);
			/* start TX/RX */
			res = dspi_data.chip.ops->txrx8(&dspi_data.chip, tx, rx,
							len);
			if (res) {
