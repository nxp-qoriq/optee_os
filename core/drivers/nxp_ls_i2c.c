...
static const char *i2c_controller_map[I2C_CTRL_NUM] = {
 ...
 * in the I2C signals.
 */
static const struct i2c_clock_divisor_pair
	i2c_clock_divisor_glitch_enabled[] = {
 ...
 */
static const struct i2c_clock_divisor_pair
	i2c_clock_divisor_glitch_disabled[] = {
...
static uint8_t i2c_get_ibfd(vaddr_t base, uint16_t clock_divisor)
{
	...
	const struct i2c_clock_divisor_pair *clock_divisor_pair;
	uint32_t clock_dvisor_pair_size = 0;
	uint32_t index = 0;

	if (io_read8((vaddr_t)&regs->ibdbg) & I2C_IBDBG_GLFLT_EN) {
		clock_divisor_pair = i2c_clock_divisor_glitch_enabled;
		clock_dvisor_pair_size = ARRAY_SIZE
			(i2c_clock_divisor_glitch_enabled);
	} else {
...
static TEE_Result i2c_read(struct i2c_regs *regs, uint32_t slave_address,
	...
	for (index = 0; index < operation->length_in_bytes; index++) {
		...
		if (index == (operation->length_in_bytes - 2)) {
			...
			if (!is_last_operation) {
				...
				i2c_io_or8((vaddr_t)&regs->ibcr, I2C_IBCR_TXRX);
			} else {
				// Generate Stop Signal
				i2c_io_and8((vaddr_t)&regs->ibcr,
					    ~(I2C_IBCR_MSSL | I2C_IBCR_TXRX));
				status = i2c_bus_test_bus_busy
					(regs, I2C_BUS_TEST_IDLE);
				if (status != TEE_SUCCESS)
					return status;
			}
		}
...
static TEE_Result i2c_write(struct i2c_regs *regs, uint32_t slave_address,
			    struct i2c_operation *operation)
{
	TEE_Result status = TEE_ERROR_GENERIC;
	uint32_t index = 0;
			...
			struct i2c_reg_request *request_packet)
{
	...
	for (index = 0, operation = request_packet->operation;
	     index < request_packet->operation_count; index++, operation++) {
