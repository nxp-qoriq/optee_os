...
static TEE_Result gpio_test_suite(void)
{
	...
	if (status == TEE_SUCCESS) {
		DMSG("GPIO Base: 0x%lx", data.gpio_base);

		for (unsigned int i = 0; i < ARRAY_SIZE(gpio_pin); i++) {
			/* Set the GPIO pin direction as output */
			data.chip.ops->set_direction(&data.chip, gpio_pin[i],
						     GPIO_TEST_DIR_OUT);
			EMSG("Get the direction of PIN %d as %d", gpio_pin[i],
			     data.chip.ops->get_direction(&data.chip,
							  gpio_pin[i]));

			/* Set the GPIO pin direction as input */
			data.chip.ops->set_direction(&data.chip,
						     gpio_pin[i ^ 1],
						     GPIO_TEST_DIR_IN);
			EMSG("Get the direction of PIN %d as %d",
			     gpio_pin[i ^ 1],
			     data.chip.ops->get_direction(&data.chip,
							  gpio_pin[i ^ 1]));
			...
			/* If an input pin, read the level (low or high) */
			EMSG("Read the value on PIN %d and read value: %d",
			     gpio_pin[i],
			     data.chip.ops->get_value(&data.chip, gpio_pin[i]));
			EMSG("Read the value on PIN %d and read value: %d",
			     gpio_pin[i ^ 1],
			     data.chip.ops->get_value(&data.chip,
						      gpio_pin[i ^ 1]));

			/* check if values at both PINs
			 * (PIN 31 and PIN 30) are same or not
			 */
			if (data.chip.ops->get_value(&data.chip, gpio_pin[i]) ==
			    data.chip.ops->get_value(&data.chip,
						     gpio_pin[i ^ 1]))
 ...
 *
 * gpio_test_suite runs some loopback tests, so the GPIO module will test
